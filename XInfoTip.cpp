...
#define CX_ROUNDED				6		// Tip horizontal roundness
...
#define CX_ICON_MARGIN			5		// Width of margin between icon and tip text
*/
#define DEFAULT_SHOW_DELAY		50		// Default show delay (ms)

#define TIMER_HIDE				1500		// Hide timer (ms)
...
void CXInfoTip::OnTimer( UINT nIDEvent ) 
{
	...
	case timerShow:
		...
		if (m_ptOrigin.x + CX_ROUNDED + WindowSize.cx <= m_ixScreen)
		{
			iShowX = m_ptOrigin.x + CX_ROUNDED;
		}
		else
		{
			iShowX = m_ptOrigin.x  - WindowSize.cx - m_szCompWindowSize.cx;
		}
		if (m_ptOrigin.y + WindowSize.cy <= m_iyScreen)
		{
			iShowY = m_ptOrigin.y;
		}
		else
		{
			iShowY = m_iyScreen - WindowSize.cy;
		}
		SetWindowPos(&wndTop,
			iShowX, 
			iShowY, 
...
void CXInfoTip::InformCompWindowSize(CPoint pt)
{
	m_szCompWindowSize.cx = pt.x;
	m_szCompWindowSize.cy = pt.y;
}